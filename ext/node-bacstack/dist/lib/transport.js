...
class Transport extends events_1.EventEmitter {
        ...
        try {
            ...
            this._server.on('message', (msg, rinfo) => {
                ...
                if (port === DEFAULT_BACNET_PORT)
                    ...
                    this.emit('message', msg, `${rinfo.address}:${rinfo.port || DEFAULT_BACNET_PORT}`);
            });
    ...
    send(buffer, offset, receiver) {
        ...
        this._server.send(buffer, 0, offset, port || this._settings.port || DEFAULT_BACNET_PORT, address);
    }
    open() {
